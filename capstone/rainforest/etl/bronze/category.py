...
class CategoryBronzeETL(TableETL):
    ...
    def extract_upstream(self) -> List[ETLDataSet]:
        ...
        category_data = self.spark.read.jdbc(
    ...
    def transform_upstream(self, upstream_datasets: List[ETLDataSet]) -> ETLDataSet:
        ...
        transformed_data = category_data.withColumn("etl_inserted", lit(current_timestamp))
    ...
    def validate(self, data: ETLDataSet) -> bool:
        # Perform any necessary validation checks on the category data
        return True

    def load(self, data: ETLDataSet) -> None:
        category_data = data.curr_data

        # Write the category data to the Delta Lake table
        category_data.write.format(data.data_format).mode("overwrite").partitionBy(
            data.partition_keys
    ...
    def read(self, partition_keys: Optional[List[str]] = None) -> ETLDataSet:
        ...
        category_data = self.spark.read.format(self.data_format).load(self.storage_path)
