...
class BuyerBronzeETL(TableETL):
    ...
    def transform_upstream(self, upstream_datasets: List[ETLDataSet]) -> ETLDataSet:
        ...
        transformed_data = buyer_data.withColumn("etl_inserted", lit(current_timestamp))
    ...
    def load(self, data: ETLDataSet) -> None:
        ...
        buyer_data.write.format(data.data_format).mode("overwrite").partitionBy(
            data.partition_keys
    ...
    def read(self, partition_keys: Optional[List[str]] = None) -> ETLDataSet:
        ...
        buyer_data = self.spark.read.format(self.data_format).load(self.storage_path)
