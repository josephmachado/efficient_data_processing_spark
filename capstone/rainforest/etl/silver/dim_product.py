...
class DimProductSilverETL(TableETL):
    def __init__(
        self,
        spark: SparkSession,
        upstream_table_names: Optional[List[Type[TableETL]]] = [ProductBronzeETL, BrandBronzeETL, ManufacturerBronzeETL],
    ...
    def transform_upstream(self, upstream_datasets: List[ETLDataSet]) -> ETLDataSet:
        ...
        common_columns = set(product_data.columns).intersection(brand_data.columns)

        # Rename common columns in product_data to avoid conflicts
        product_data = product_data.selectExpr(
            *[f"`{col}` as product_{col}" if col in common_columns and col != "brand_id" else col for col in product_data.columns]
        )

        # Rename common columns in brand_data to avoid conflicts
        brand_data = brand_data.selectExpr(
            *[f"`{col}` as brand_{col}" if col in common_columns and col != "brand_id" else col for col in brand_data.columns]
        )

        # Join product_data with brand_data on brand_id
        dim_product_data = product_data.join(
            brand_data, product_data["brand_id"] == brand_data["brand_id"], "left"
        ).drop(brand_data["brand_id"])

        # Get common columns in both product_data and manufacturer_data
        common_columns = set(dim_product_data.columns).intersection(manufacturer_data.columns)

        # Rename common columns in dim_product_data to avoid conflicts
        dim_product_data = dim_product_data.selectExpr(
            *[f"`{col}` as product_{col}" if col in common_columns and col not in ["brand_id", "manufacturer_id"] else col for col in dim_product_data.columns]
        )

        # Rename common columns in manufacturer_data to avoid conflicts
        manufacturer_data = manufacturer_data.selectExpr(
            *[f"`{col}` as manufacturer_{col}" if col in common_columns and col != "manufacturer_id" else col for col in manufacturer_data.columns]
        )

        # Join dim_product_data with manufacturer_data on manufacturer_id
        dim_product_data = dim_product_data.join(
            manufacturer_data, dim_product_data["manufacturer_id"] == manufacturer_data["manufacturer_id"], "left"
    ...
    def load(self, data: ETLDataSet) -> None:
        ...
        dim_product_data.write.option("mergeSchema", "true").format(data.data_format).mode("overwrite").partitionBy(
            data.partition_keys
    ...
    def read(self, partition_keys: Optional[List[str]] = None) -> ETLDataSet:
        ...
        dim_product_data = self.spark.read.format(self.data_format).load(self.storage_path)
