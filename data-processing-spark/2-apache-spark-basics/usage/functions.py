...
def run_code(spark):
    ...
    print("=======================================")
    concat_df = spark.table("orders").select(concat(col("clerk"), lit("-"), col("orderpriority")).alias("concatenated")).limit(5)
    concat_df.show()

    print("=======================================")
    print("Splitting clerk column by '#' delimiter")
    print("=======================================")
    split_df = spark.table("orders").select(split(col("clerk"), "#").alias("split_clerk")).limit(5)
    split_df.show()

    print("=======================================")
    print("Extracting first 5 characters of clerk column")
    print("=======================================")
    substr_df = spark.table("orders").select(col("clerk"), substring(col("clerk"), 1, 5).alias("substr_clerk")).limit(5)
    substr_df.show()
    ...
    date_diff_df = fake_df.select(
        ...
        months_between(lit('2022-11-05'), lit('2022-10-01')).alias("diff_in_months"),
        (months_between(lit('2022-11-05'), lit('2022-10-01')) / 12).alias("diff_in_years"),
    ...
    print("=======================================")
    date_interval_df = fake_df.select(date_add(lit('2022-11-05'), 10).alias("new_date"))
    date_interval_df.show()

    print("=======================================")
    print("Parsing '11-05-2023' as date")
    print("=======================================")
    parsed_date_df = fake_df.select(to_date(lit('11-05-2023'), 'MM-dd-yyyy').alias("parsed_date"))
    parsed_date_df.show()

    print("=======================================")
    print("Formatting orderdate to first day of month")
    print("=======================================")
    first_month_df = spark.table("orders").select(date_format(col("orderdate"), 'yyyy-MM-01').alias("first_month_date")).limit(5)
    first_month_df.show()
    ...
    print("=======================================")
    round_df = fake_df.select(round(lit(100.102345), 2).alias("rounded_number"))
    round_df.show()

    print("=======================================")
    print("Calculating absolute values of -100 and 100")
    print("=======================================")
    abs_df = fake_df.select(abs(lit(-100)).alias("abs_negative"), abs(lit(100)).alias("abs_positive"))
    abs_df.show()

    print("=======================================")
    print("Calculating ceiling and floor of 100.1")
    print("=======================================")
    ceil_floor_df = fake_df.select(ceil(lit(100.1)).alias("ceiling"), floor(lit(100.1)).alias("floor"))
    ceil_floor_df.show()
